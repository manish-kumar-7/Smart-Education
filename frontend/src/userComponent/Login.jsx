...
function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    if (!email || !password) return handleError("Please enter all fields");
    ...
    try {
      ...
      if (success) {
        ...
        setTimeout(() => {
          if (role === "owner") navigate("/alluser");
          else if (role === "admin") navigate("/admindashboard");
          else navigate("/profile", { state: { username, role } });
        }, 2000);
      } else if (error) {
  ...
  const roleIndex = { user: 0, admin: 1, owner: 2 }[role];

  return (
    <div
      className="min-h-screen flex items-center justify-center px-4 bg-cover bg-center relative"
      style={{ backgroundImage: "url('./public/image1.jpg')" }}
    >
      ...
      <div className="relative w-full max-w-md bg-white/20 backdrop-blur-lg border border-white/30 rounded-2xl shadow-2xl p-8">
        ...
        <div className="flex justify-center mb-6">
          <div className="relative flex border border-white/40 rounded-full overflow-hidden bg-white/20 backdrop-blur-md">
            {/* Sliding background */}
            <span
              className="absolute top-0 left-0 h-full w-1/3 bg-gradient-to-r from-cyan-200 via-sky-200 to-blue-300 rounded-full transition-transform duration-300"
              style={{
            ...
            {["user", "admin", "owner"].map((r) => (
              <button
                ...
                className={`relative px-5 py-2 text-sm font-medium transition-colors duration-300 ${
                  role === r ? "text-gray-800" : "text-gray-200"
                }`}
              >
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <div className="relative w-full">
            ...
            <button
              ...
              className="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 flex items-center justify-center rounded-full bg-pink-300 hover:bg-white/20 text-sm transition-all duration-200"
        ...
        <p className="mt-6 text-center text-gray-200 text-sm">
          ...
          <Link to="/signup" className="text-pink-400 font-medium hover:underline">
            Signup
          </Link>
        </p>
      </div>
